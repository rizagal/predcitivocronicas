...
if(selected == 'Consulta Resultado Indicadores Atencion al Usuario'):
    ...
    with col1:
       st.dataframe(df_selection.style.apply(color_negative_red, subset=['PORCENTAJE']).format({"PORCENTAJE": "{:.}"}),hide_index=True,height=450,use_container_width=True,column_order=("EVALUACION","MES","PORCENTAJE","NOMBREIPS"))
